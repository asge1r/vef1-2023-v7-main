...
function formatProduct(product, quantity = undefined) {
  let formattedString = `${product.name} — ${product.price.toLocaleString('is-IS')} kr.`;
  if (quantity !== undefined) {
    const totalPrice = product.price * quantity;
    formattedString += ` — ${quantity}x${product.price.toLocaleString('is-IS')} kr. samtals ${totalPrice.toLocaleString('is-IS')} kr.`;
  }
